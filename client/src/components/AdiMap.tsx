    ...
    <div className="rounded-xl overflow-hidden border border-border shadow-md h-[600px] bg-white relative z-0">
      <MapContainer center={center} zoom={zoom} scrollWheelZoom={false} className="h-full w-full">
        ...
        {safeScores.map((item, idx) => {
          ...
          return (
            <CircleMarker
              key={`${item.state}-${item.district}-${item.pincode}-${idx}`}
              center={coords}
              radius={6 + score / 15}
              pathOptions={{
            ...
            >
              <Popup>
                <div className="p-1 text-xs">
                  <b>{item.district}, {item.state}</b>
                  <div>ADI: {score.toFixed(2)}</div>
                  <div>Enrol Dev: {Number(item.enrolmentDev || 0).toFixed(2)}</div>
                  <div>Age Shift: {Number(item.ageShift || 0).toFixed(2)}</div>
                </div>
              </Popup>
              <Tooltip direction="top" offset={[0, -10]} opacity={1}>
                {item.district} (ADI: {score.toFixed(1)})
              </Tooltip>
            </CircleMarker>
          );
        })}
